...
import ImportIlawDocxModal from '../ImportIlawDocxModal';
...
interface TeacherCurriculumViewProps {
  topics?: Topic[];
  initialSubTab?: CurriculumSubTab | 'my';
...
export default function TeacherCurriculumView({
  ...
  const normalizedInitialTab: CurriculumSubTab = 
    initialSubTab === 'my' ? 'ilaw' : 
    (initialSubTab as CurriculumSubTab) || 'overview';

  const [subTab, setSubTab] = useState<CurriculumSubTab>(normalizedInitialTab);

  useEffect(() => {
    if (initialSubTab) {
      setSubTab(initialSubTab === 'my' ? 'ilaw' : (initialSubTab as CurriculumSubTab));
    }
  }, [initialSubTab]);
  ...
  const [editingIlawLesson, setEditingIlawLesson] = useState<LessonPlan | null>(null);
  ...
  const [isImportDocxModalOpen, setIsImportDocxModalOpen] = useState(false);
  ...
  // Inline "Create Lesson" form state
  ...
  // Inline DOCX Dropzone state in "import" subtab
  const [inlineDocxParsing, setInlineDocxParsing] = useState(false);
  const [inlineDocxResults, setInlineDocxResults] = useState<ParsedDocxLesson[]>([]);
  const [inlineDocxError, setInlineDocxError] = useState<string | null>(null);
  const fileInputRef = React.useRef<HTMLInputElement>(null);

  // Filter lessons based on subTab and filters
  const filteredLessons = useMemo(() => {
    return ilawLessons.filter(l => {
      // Sub-tab filter
      ...
      if (selectedTerm !== 'All') {
        ...
        const filterTerm = selectedTerm.toLowerCase();
  ...
  }, [ilawLessons, subTab, selectedTerm, selectedTopicFilter, searchQuery, topics]);

  // Handlers for Add, Edit, Delete, Duplicate
  const handleOpenCreateModal = () => {
    setEditingIlawLesson(null);
    setIsIlawModalOpen(true);
  };

  const handleEditLesson = (lesson: LessonPlan) => {
    setEditingIlawLesson(lesson);
    setIsIlawModalOpen(true);
  };

  const handleSaveModalLesson = async (savedData: Partial<LessonPlan> & { title: string }) => {
    const isEdit = !!editingIlawLesson;
  ...
  const handleImportDocxLessons = async (importedLessons: (Partial<LessonPlan> & { title: string })[]) => {
    for (const lesson of importedLessons) {
      await saveIlawLesson(lesson);
    }
    showToast(`Successfully imported ${importedLessons.length} ILAW lesson plan${importedLessons.length > 1 ? 's' : ''} from DOCX!`);
    setSubTab('ilaw');
  };

  // Inline "Create Lesson" sub-tab submit
  ...
  // Inline DOCX File upload handler for "import" tab
  const handleInlineDocxFiles = async (files: FileList | File[]) => {
    ...
    if (docxFiles.length === 0) {
      setInlineDocxError('Please upload valid Microsoft Word (.docx) documents.');
      return;
    }

    setInlineDocxParsing(true);
    setInlineDocxError(null);
    ...
    try {
      ...
      setInlineDocxResults(prev => [...prev, ...results]);
    } catch (err: any) {
      console.error('Error parsing docx file:', err);
      setInlineDocxError(err.message || 'Failed to parse DOCX file.');
    } finally {
      setInlineDocxParsing(false);
    }
  };

  const handleSaveInlineDocxResults = async (targetStatus: 'Published' | 'Draft') => {
    if (inlineDocxResults.length === 0) return;
    try {
      for (const item of inlineDocxResults) {
        await saveIlawLesson({
          ...item.lesson,
          status: targetStatus,
          updatedAt: targetStatus === 'Published' ? 'Imported & Published from DOCX' : 'Imported Draft from DOCX'
        });
      }
      showToast(`Saved ${inlineDocxResults.length} ILAW lesson plan${inlineDocxResults.length > 1 ? 's' : ''} from DOCX!`);
      setInlineDocxResults([]);
      setSubTab(targetStatus === 'Published' ? 'published' : 'drafts');
    } catch (err: any) {
      showToast(err.message || 'Failed to save imported lessons', 'error');
    }
  };
  ...
  const handleResetBaseline = async () => {
    ...
    try {
      ...
      showToast('Restored all ILAW lessons to DepEd baseline exemplars.');
      setIsResetModalOpen(false);
    } catch (err: any) {
      showToast(err.message || 'Failed to reset lessons', 'error');
    } finally {
  ...
  const publishedCount = ilawLessons.filter(l => l.status !== 'Draft').length;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-emerald-950 p-6 sm:p-8 rounded-3xl text-white shadow-xl border border-emerald-500/20 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="bg-emerald-400 text-slate-950 text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full tracking-wider flex items-center gap-1">
                ...
                <span>Unified Curriculum & Lesson Hub</span>
              </span>
              <span className="bg-white/10 text-emerald-200 text-[10px] font-bold px-2 py-0.5 rounded-full">
                DepEd MATATAG / D.O. No. 016, s. 2024
              </span>
              <span className="bg-indigo-500/20 text-indigo-200 text-[10px] font-bold px-2 py-0.5 rounded-full">
                ILAW 4-Pillar Pedagogical Standard
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
              Curriculum & Lesson Management Studio
            </h1>
            <p className="text-xs sm:text-sm text-slate-300 max-w-2xl leading-relaxed">
              Consolidated command center for SHS General Mathematics curriculum standards, MELCs progression, and authoring, importing, editing, and publishing official DepEd ILAW Daily Lesson Plans.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5 shrink-0 flex-wrap">
            <button
              ...
              className="px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black rounded-2xl text-xs flex items-center gap-2 shadow-lg shadow-emerald-500/20 transition-all cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
            >
            ...
            <button
              type="button"
              onClick={() => setIsImportDocxModalOpen(true)}
              className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl text-xs flex items-center gap-2 shadow-lg shadow-indigo-600/20 transition-all cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
            >
      ...
      {/* SUB-TAB 1: CURRICULUM OVERVIEW */}
      {subTab === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs space-y-1">
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Total Curriculum Units</span>
              <h3 className="text-2xl font-black text-slate-900">{topics.length} Units</h3>
              <p className="text-xs text-slate-500">Across 4 DepEd Quarters</p>
            </div>

            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs space-y-1">
              <span className="text-[10px] font-black uppercase text-emerald-600 tracking-wider">ILAW Lesson Plans</span>
              <h3 className="text-2xl font-black text-slate-900">{ilawLessons.length} Plans</h3>
              <p className="text-xs text-emerald-700 font-bold">{publishedCount} Published • {draftCount} Drafts</p>
            </div>

            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs space-y-1">
              <span className="text-[10px] font-black uppercase text-indigo-600 tracking-wider">Target Competencies</span>
              <h3 className="text-2xl font-black text-slate-900">18 MELCs</h3>
              <p className="text-xs text-slate-500">100% Core Standard Alignment</p>
            </div>

            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs space-y-1">
              <span className="text-[10px] font-black uppercase text-amber-600 tracking-wider">DepEd Policy Reference</span>
              <h3 className="text-base font-black text-slate-900">D.O. No. 016, s. 2024</h3>
              <p className="text-xs text-slate-500">ILAW Lesson Framework</p>
            </div>
          </div>

          {/* Quarters Scope & Sequence Grid */}
          <div className="space-y-3">
            <h3 className="text-xs font-black text-slate-500 uppercase tracking-wider">
              Quarterly Scope & Sequence Pacing
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {['Term 1', 'Term 2', 'Term 3', 'Term 4'].map((quarter, qIdx) => {
                const quarterTopics = topics.filter(t => t.term === quarter || (!t.term && qIdx === 0));
                const quarterPlans = ilawLessons.filter(l => l.term === quarter);

                return (
                  <div key={quarter} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                      <div>
                        <span className="text-[10px] font-black uppercase text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded border border-emerald-100">
                          Quarter {qIdx + 1}
                        </span>
                        <h4 className="text-base font-black text-slate-900 mt-1">{quarter} Curriculum Units</h4>
                      </div>
                      <span className="text-xs font-bold text-slate-400">
                        {quarterTopics.length} Units • {quarterPlans.length} DLPs
                      </span>
                    </div>

                    <div className="space-y-2">
                      {quarterTopics.map((topic, tIdx) => {
                        const plan = ilawLessons.find(l => l.topicId === topic.id);

                        return (
                          <div key={topic.id} className="p-3 bg-slate-50 rounded-2xl border border-slate-200/70 flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2.5 min-w-0">
                              <span className="w-6 h-6 rounded-lg bg-indigo-100 text-indigo-900 font-black text-xs flex items-center justify-center shrink-0">
                                {tIdx + 1}
                              </span>
                              <div className="min-w-0">
                                <h5 className="text-xs font-bold text-slate-900 truncate">{topic.title}</h5>
                                <span className="text-[10px] text-slate-400 font-mono">{topic.week || `Week ${tIdx + 1}`}</span>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                              {plan ? (
                                <button
                                  type="button"
                                  onClick={() => handleEditLesson(plan)}
                                  className="px-2.5 py-1 bg-white hover:bg-emerald-50 text-emerald-700 border border-slate-200 rounded-lg text-[10px] font-black transition-colors cursor-pointer"
                                >
                                  View DLP
                                </button>
                              ) : (
                                <button
                                  type="button"
                                  onClick={handleOpenCreateModal}
                                  className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg text-[10px] font-black transition-colors cursor-pointer"
                                >
                                  + Create DLP
                                </button>
                              )}
      ...
      {/* SUB-TAB 2, 5, 6: ILAW LESSONS LIST, DRAFTS, PUBLISHED */}
      {(subTab === 'ilaw' || subTab === 'drafts' || subTab === 'published') && (
            ...
            <div className="py-12 px-6 bg-white rounded-3xl border border-slate-200 text-center space-y-4 max-w-lg mx-auto">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  {searchQuery || selectedTerm !== 'All' || selectedTopicFilter !== 'All'
                    ? 'No lesson plans match your current search and filters. Clear filters or create a new lesson.'
                    : subTab === 'drafts'
                    ? 'You have no draft lesson plans. All your lessons are currently published!'
                    : 'There are currently no lesson plans here. Click below to author or import from DOCX!'}
                </p>
              </div>
      ...
      {/* SUB-TAB 4: IMPORT ILAW LESSONS FROM DOCX */}
      {subTab === 'import' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="px-2.5 py-0.5 bg-indigo-100 text-indigo-800 text-[10px] font-black uppercase rounded-md">
                  Mammoth DOCX Extractor
                </span>
                <span className="text-xs text-slate-400 font-bold">DepEd Order No. 016, s. 2024</span>
              </div>
              <h3 className="text-lg font-black text-slate-900">
                Import DepEd ILAW Lesson Plans from Word (.docx) Files
              </h3>
              <p className="text-xs text-slate-500">
                Upload your DepEd Daily Lesson Plans (DLP) or Daily Lesson Logs (DLL) in .docx format. Our intelligent parser automatically maps text into the 4 ILAW pillars.
              </p>
            </div>
            ...
            <button
              type="button"
              onClick={() => setIsImportDocxModalOpen(true)}
              className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl text-xs flex items-center gap-1.5 cursor-pointer shadow-xs"
            >
              <Upload className="w-4 h-4" />
              <span>Open Batch Upload Modal</span>
            </button>
          </div>
          ...
          <div
            ...
            className="border-2 border-dashed border-indigo-200 hover:border-indigo-400 bg-indigo-50/30 hover:bg-indigo-50/60 rounded-3xl p-8 sm:p-12 text-center transition-all cursor-pointer space-y-4"
          >
            <input
              type="file"
              ref={fileInputRef}
              ...
              onChange={(e) => e.target.files && handleInlineDocxFiles(e.target.files)}
              className="hidden"
            />

            <div className="w-16 h-16 bg-indigo-100 text-indigo-600 rounded-2xl flex items-center justify-center mx-auto shadow-xs">
              {inlineDocxParsing ? (
            ...
            <div className="space-y-1">
              <h4 className="text-base font-black text-slate-900">
                {inlineDocxParsing ? 'Extracting ILAW Structure from Word...' : 'Click to Browse or Drag & Drop .docx Files Here'}
              </h4>
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                Upload one or multiple DepEd General Mathematics Lesson Plans (.docx). The parser will automatically extract Intentions, Learning Experience, Assessing Learning, and Ways Forward.
              </p>
            </div>
          ...
          {/* Parsed Items Queue */}
          {inlineDocxResults.length > 0 && (
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-black text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                  <FileCheck className="w-4 h-4 text-emerald-600" />
                  <span>Parsed Lesson Plans Ready to Import ({inlineDocxResults.length})</span>
                </h4>

                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => handleSaveInlineDocxResults('Draft')}
                  ...
                  <button
                    type="button"
                    onClick={() => handleSaveInlineDocxResults('Published')}
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl text-xs transition-colors cursor-pointer shadow-xs flex items-center gap-1"
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>Import & Publish All</span>
                  </button>
                </div>
              </div>

              <div className="grid gap-3">
                {inlineDocxResults.map((res, ri) => (
                  <div key={ri} className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 rounded-md bg-emerald-100 text-emerald-900 font-black text-[10px] flex items-center justify-center">
                          {ri + 1}
                        </span>
                        <h5 className="font-black text-slate-900 text-xs">{res.lesson.title}</h5>
                        <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-800 rounded text-[9px] font-black">
                          {res.confidence}% Match
                        </span>
                      </div>
                      <p className="text-[11px] text-slate-500">
                        {res.lesson.term} • {res.lesson.week} • Competencies: {res.lesson.learningCompetencies?.join(', ') || 'Auto-generated'}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        type="button"
                        onClick={() => setInlineDocxResults(prev => prev.filter((_, i) => i !== ri))}
      ...
      {/* SUB-TAB 7: MELCs COMPETENCIES */}
      {subTab === 'competencies' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div>
              <h3 className="text-base font-black text-slate-900">
                Senior High School General Mathematics MELCs Roster
              </h3>
              <p className="text-xs text-slate-500">
                Official Department of Education Most Essential Learning Competencies mapped across curriculum topics and ILAW lessons.
              </p>
            </div>
            <span className="text-xs font-bold text-indigo-700 bg-indigo-50 px-3 py-1 rounded-xl border border-indigo-100">
              DepEd MELCs Standard
            </span>
          </div>

          <div className="grid gap-2.5 pt-1">
            {topics.flatMap((t, tIdx) => {
              const livePlan = ilawLessons.find(l => l.topicId === t.id && l.status !== 'Draft');
              const comps = livePlan?.learningCompetencies || t.lessonPlan?.learningCompetencies || [
                `M11GM-I${String.fromCharCode(97 + (tIdx % 10))}-1: Demonstrates understanding of core properties for ${t.title}.`
              ];

              return comps.map((comp, cIdx) => ({
                code: comp.split(':')[0] || `M11GM-${tIdx + 1}`,
                description: comp.includes(':') ? comp.split(':')[1] : comp,
                topic: t,
                fullText: comp
              }));
            }).map((item, idx) => (
              <div
                key={idx}
                className="p-4 bg-slate-50 hover:bg-emerald-50/40 rounded-2xl border border-slate-200/80 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 rounded-lg bg-emerald-100 text-emerald-900 font-black text-[10px] flex items-center justify-center shrink-0">
                      {idx + 1}
                    </span>
                    ...
                    <span className="text-[10px] font-bold text-slate-500 bg-white px-2 py-0.5 rounded border border-slate-200">
                      {item.topic.term || 'Term 1'} • {item.topic.week || 'Week 1'}
                    </span>
                  </div>
                  <p className="text-xs text-slate-800 font-medium pl-8">
                    {item.description}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const plan = ilawLessons.find(l => l.topicId === item.topic.id);
                      if (plan) handleEditLesson(plan);
      ...
      {/* SUB-TAB 8: CURRICULUM PROGRESSION MAP */}
      {subTab === 'map' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          <div className="border-b border-slate-100 pb-3">
            <h3 className="text-base font-black text-slate-900">
              Curriculum Progression & Prerequisite Dependency Map
            </h3>
            <p className="text-xs text-slate-500">
              Visual learning pathway illustrating prerequisite mastery flow across Senior High School General Mathematics.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {topics.map((topic, idx) => {
              ...
              return (
                <div key={topic.id} className="p-5 bg-slate-50 rounded-3xl border border-slate-200 space-y-3 relative overflow-hidden">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                      Step {idx + 1} • {topic.term || 'Term 1'}
                    </span>
                    <span className="text-[10px] font-bold text-slate-400">{topic.week}</span>
                  </div>

                  <div>
                    <h4 className="font-black text-slate-900 text-sm">{topic.title}</h4>
                    <p className="text-xs text-slate-500 mt-0.5 line-clamp-2">{topic.description}</p>
                  </div>

                  <div className="pt-2 border-t border-slate-200/80 space-y-1.5">
                    <span className="text-[10px] font-bold text-slate-500 uppercase block">Prerequisites:</span>
                    <div className="flex items-center gap-1 flex-wrap">
                      {(topic.lessonPlan?.prerequisites || ['Foundational Algebra', 'Real Numbers']).map((prereq, pi) => (
                        <span key={pi} className="text-[10px] font-bold bg-white text-slate-700 px-2 py-0.5 rounded-md border border-slate-200">
                          {prereq}
                        </span>
                      ))}
                    </div>
                  ...
                  {plan && (
                    <button
                      type="button"
                      onClick={() => handleEditLesson(plan)}
                      className="w-full mt-2 py-1.5 bg-white hover:bg-emerald-50 text-emerald-700 font-bold text-xs rounded-xl border border-slate-200 transition-colors cursor-pointer"
                    >
                      Open Linked ILAW DLP
                    </button>
                  )}
                </div>
              );
      ...
      {/* SUB-TAB 9: STRUCTURE TREE HIERARCHY */}
      {subTab === 'hierarchy' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-4">
          <div className="border-b border-slate-100 pb-3">
            <h3 className="text-base font-black text-slate-900">
              Department of Education Curriculum Structure Hierarchy
            </h3>
            <p className="text-xs text-slate-500">
              Interactive tree visualization of Grade 11 General Mathematics strands, academic terms, units, and learning competency codes.
            </p>
          </div>
          <CurriculumHierarchyBrowser topics={topics} />
        </div>
      )}

      {/* SUB-TAB 10: TEMPLATES LIBRARY */}
      {subTab === 'templates' && (
      ...
      {/* Import from DOCX Modal */}
      <ImportIlawDocxModal
        isOpen={isImportDocxModalOpen}
        topics={topics}
        onClose={() => setIsImportDocxModalOpen(false)}
        onImportComplete={handleImportDocxLessons}
      />
